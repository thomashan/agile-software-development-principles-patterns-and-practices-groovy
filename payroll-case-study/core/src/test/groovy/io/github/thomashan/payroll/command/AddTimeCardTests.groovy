...
class AddTimeCardTests implements CommandTests {
    private LocalDate today = LocalDate.now()

    @Test
    void "time card command on nonexistent employee should throw an error"() {
        assertThrows(RuntimeException, { new AddTimeCard(employeeId, today, 8).execute() })
    }
    ...
    void "time card command on hourly employee should execute without error"() {
        ...
        new AddTimeCard(employeeId, today, 8).execute()
